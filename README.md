...
**时间归属**：`--since` / `--until` 的 `YYYY-MM-DD` 日期按**本地时区**解析日界线（与官方端的日统计一致）。过滤时 token 用量按**每次请求的实际发生时间**逐条归属。跨天会话（如 06-08 启动、一直跑到 06-09 的会话）会按消息时间戳把用量拆分到对应日期，而不是整体算给会话启动日，与各工具官方端的日统计口径一致。无逐条 usage 数据的会话回退为按会话开始时间整体取舍。
